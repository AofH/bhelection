$( document ).ready(function() {
  ...
  $.ajax({type:"GET",
   ...
   $.ajax({type:"GET",
          url:"/visualization/parties"
  }).done(function(data){

    var margin = {left:200, right: 40, top:40, bottom:40};
    var width = 1000 - margin.left - margin.right;
    ...
    var svg = d3.select("#party-breakdown-graph")
                .append('svg')
                .attr("width", width+margin.left+margin.right)
                .attr("height", height + margin.top + margin.bottom)
                .append('g')
    ...
    var bar = svg.selectAll("g.bar")
                 ...
                 .append('g')
                 .attr("class", "bar")
                 .attr("transform", function(d){ return "translate(0,"+ y(d.party) +")"});
